...
class SerialCasterTest {
     ...
     */
    @Test fun testBreakIfDecodedIsTooShort()
    {
        assertFailsWith<SerialCasterException>(
            message = "If the decoded serial is too short (<=2) it should throw an exception",
            block = {
                SerialCaster.decode("A", SEED, ALPHANUMERIC.toCharArray())
            }
        )
    }
     ...
     */
    @Test fun testBreakIfDecodedCharListIsDifferentThanTheOneUsedForEncoding()
    {
        assertFailsWith<SerialCasterException>(
     ...
     */
    @Test fun testBreakIfLengthIsNotHighEnough()
    {
        assertFailsWith<SerialCasterException>(
            message = "Tests if encode would throw an error if serial length is not high enough",
            block = {
                // This should break.
     ...
     */
    @Test fun testBreakIfCharListIsTooShort()
    {
        assertFailsWith<SerialCasterException>(
     ...
     */
    @Test fun testEncodeAndDecodeWithRandomValues()
    {
     ...
     */
    @Test fun testSpeedTestOnCouponGeneration()
    {
        ...
        val measured = measureNanoTime {
            ...
            while (i <= numberOfCoupons) {
                ...
                coupons.add(SerialCaster.encode(
                    randomInteger, SEED, LENGTH, ALPHANUMERIC.toCharArray()
                ))
                i++
            }
        }
     ...
     */
    @Test fun testSpeedTestOnCouponDecode()
    {
        val acceptableTimeInSeconds = 4
        ...
        val measured = measureNanoTime {
            i = 1
            while (i < numberOfCoupons) {
                decodedCoupons.add(SerialCaster.decode(
                    coupons[i], SEED, ALPHANUMERIC.toCharArray()
                ))
                i++
            }
        }
